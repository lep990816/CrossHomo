...
def train(args, params1, params2, params3):
    global best
    ...
    if args.optim == 'adam':
        optimizer1 = optim.Adam(models[0].parameters(), lr=params1.learning_rate)
        optimizer2 = optim.Adam(models[1].parameters(), lr=params2.learning_rate)
    ...
    if args.load_latest == 1:
        ...
        for id in range(3):
            ...
            for params in optimizers[id].param_groups:  # 遍历Optimizer中的每一组参数
                params['lr'] = 1e-5
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch_value in enumerate(train_loader):
            ...
            input1 = batch_value[0].float()
            input2 = batch_value[1].float()
            pts1 = batch_value[2]
            target = batch_value[3].float()

            if torch.cuda.is_available():
                input1 = input1.cuda() / 255.0
                target = target.cuda()
                input2 = input2.cuda() / 255.0
                pts1_ori = pts1.cuda()
            ...
            for level in range(3):
                ...
                for level_i in range(3):
                    ...
                    H_pre_tmp = kornia.get_perspective_transform(pts1, pts2) # 512*512
                    _, _, H, W = input2_SR_gt.shape
                    H_pre_level = h_adjust(4,4,2**level_i,2**level_i,H_pre_tmp)                
                    # input2 = kornia.warp_perspective(input2.clone(), H_pre_level, (H, W))
        ...
        with torch.no_grad():
            ...
            for i, batch_value in enumerate(val_loader):
                ...
                for level in range(3):
                    ...
                    # input2 = nn.functional.interpolate(input2_SR, scale_factor=2, mode='bicubic', align_corners=False)                    input2_SR_gt = input2_SR_gt_list[level]
                    # input1 = input1[:,:,20*(2**level):116*(2**level), 20*(2**level):116*(2**level)]
                    # input2 = input2[:,:,20*(2**level):116*(2**level), 20*(2**level):116*(2**level)]
                    delta, fea_out1, fea_out2, input2_SR = model(torch.cat((input1,input2),dim=1)) 
            ...
            if np.mean(loss_homo_level[2]) < best:
                id = 0
                for model in models:
...
if __name__ == "__main__":
    ...
    train(args, params1, params2, params3)
