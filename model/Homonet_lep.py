...
class SwinTransformer_cross(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, param, norm_layer=nn.LayerNorm, use_checkpoint=False, *args, **kwargs):
        ...
        self.i_layer_cross = 0
        self.cross_layer = BasicCrossLayer(dim=int(self.embed_dim * 2 ** self.i_layer_cross),
                               input_resolution=(
                                   param.crop_size[0] // (2 ** (self.i_layer_cross + 1)),
                                   param.crop_size[1] // (2 ** (self.i_layer_cross + 1))),
                               depth=param.depths[self.i_layer_cross],
                               layer_depth=param.layer_depth[self.i_layer_cross],
                               num_heads=param.num_heads[self.i_layer_cross],
                               window_size=param.window_size,
                               mlp_ratio=self.mlp_ratio,
                               qkv_bias=param.qkv_bias, qk_scale=param.qk_scale,
                               drop=param.drop_rate, attn_drop=param.attn_drop_rate,
                               drop_path=dpr[sum(param.depths[:self.i_layer_cross]):sum(param.depths[:self.i_layer_cross + 1])],
                               norm_layer=norm_layer,
                               downsample=PatchMerging,
                               use_checkpoint=use_checkpoint)
        self.PatchMerging1 = PatchMerging(input_resolution=(
            param.crop_size[0] // (2 ** (self.i_layer_cross + 1)),
            param.crop_size[1] // (2 ** (self.i_layer_cross + 1))), dim=int(self.embed_dim * 2 ** self.i_layer_cross),
            norm_layer=norm_layer)

        self.i_layer_self = 1
        self.self_layer = BasicLayer(dim=int(self.embed_dim * 2 ** self.i_layer_self),
                                      input_resolution=(
                                          param.crop_size[0] // (2 ** (self.i_layer_self + 1)),
                                          param.crop_size[1] // (2 ** (self.i_layer_self + 1))),
                                      depth=param.depths[self.i_layer_self],
                                      layer_depth=param.layer_depth[self.i_layer_self],
                                      num_heads=param.num_heads[self.i_layer_self],
                                      window_size=param.window_size,
                                      mlp_ratio=self.mlp_ratio,
                                      qkv_bias=param.qkv_bias, qk_scale=param.qk_scale,
                                      drop=param.drop_rate, attn_drop=param.attn_drop_rate,
                                      drop_path=dpr[
                                                sum(param.depths[:self.i_layer_self]):sum(param.depths[:self.i_layer_self + 1])],
                                      norm_layer=norm_layer,
                                      downsample=PatchMerging,
                                      use_checkpoint=use_checkpoint)
        ...
        self.blocks_token_only = LayerScale_Block_CA(
                dim=self.num_features, out_dim=self.num_features, num_heads=param.num_heads[0],
                mlp_ratio=self.mlp_ratio, qkv_bias=param.qkv_bias, qk_scale=param.qk_scale,
                drop=param.drop_rate, attn_drop=param.attn_drop_rate, drop_path=0.25, norm_layer=norm_layer,
    ...
    def forward(self, x):
        ...
        x1_fea = self.feature_pyramid_extractor(x1_patch)  # + [x1_patch]
        x2_fea = self.feature_pyramid_extractor(x2_patch)  # + [x2_patch]
        fea_out1 = x1_fea
        fea_out2 = x2_fea
        # save_pic(fea_out1[1][0,0],'fea1_CNN_2')
        # save_pic(fea_out2[1][0,0],'fea2_CNN_2')
        # pdb.set_trace()

        weight_f = 0

        _, _, h_x, w_x, = x1_fea[1].shape

        x1_fea = x1_fea[1].flatten(2).transpose(1, 2)
        x2_fea = x2_fea[1].flatten(2).transpose(1, 2)

        # x_fea = torch.cat((x1_fea, x1_fea), dim=1)
        # pdb.set_trace()

        x1_cross , x2_cross = self.cross_layer(x1_fea, x2_fea)
        x = torch.cat((x1_cross, x2_cross), dim=2)
        x = self.PatchMerging1(x)
        x = self.self_layer(x)
        # print(x.size())
        # pdb.set_trace()

        query_token = self.blocks_token_only(query_token, x)
        ...
        return weight_f,fea_out1,fea_out2
...
class BasicCrossLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, layer_depth, num_heads, window_size,
        ...
        for l in range(layer_depth):
            ...
            dim_current = dim * 2 ** l
            blocks_list = [SwinCrossBlock(dim=dim_current//2, input_resolution=input_resolution_current,
