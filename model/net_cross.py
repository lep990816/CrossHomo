...
from .FlowNet_model_level import FlowNet
...
class HomoNet_lep(nn.Module):
    ...
    def forward(self, input1_HR,input2_bic_SR):
        ...
        # # ==========================super resolution======================================
        if self.params.SR == 1:
            input2_SR = self.SR(input1_HR,input2_bic_SR)
        elif self.params.SR == 0:
            input2_SR = input2_bic_SR
        elif self.params.SR == 2:
            input2_SR,conv1_HR_tmp,conv1_HR,conv1_LR = self.SR3(input1_HR,input2_bic_SR)
        # # ==========================full features======================================
        img1_patch_fea, img2_patch_fea = list(map(self.fea_extra, [input1_HR, input2_SR]))
        # img1_full_fea, img2_full_fea = list(map(self.fea_extra, [img1_full, img2_full]))
        #
        # # ========================forward ====================================
        #
        ...
        return delta,fea_out1[1],fea_out2[1],input2_SR
        # H_flow_f = (self.basis.to(forward_fea.device) * weight_f).sum(1).reshape(bs, 2, h_patch, w_patch)
